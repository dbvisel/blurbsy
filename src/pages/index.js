        ...
        return (
          <Layout>
            ...
            <section>
              <h2>What is this?</h2>
              <p>
                Blurbsy shows connections between authors who have blurbed each
                other. <Link to="/about">Find out more about this project</Link>
                .
              </p>
              <h3>
                <br />
              ...
              <AuthorList>
                {data.AuthorList.edges.sort(compareNames).map((node, index) => (
                  <AuthorName key={index}>
                    <Link to={`/author/${cleanName(node.node.data.Name)}`}>
                      {node.node.data.Name}
                    </Link>
                  </AuthorName>
                ))}
